...
describe('ContractingService', () => {
  ...
  describe('it should send mails', () => {
    it('for researcher action', async () => {
      const user = {
        fullName: 'fullName',
        singleKnownRole: Role.Researcher,
        isFromLocation: false,
      } as any as IRequestUser;
      await contractingService.handleContractSign(proposal, true, user, proposalUrl);
      expect(emailService.send).toHaveBeenCalled();
    });
    it('for location action', async () => {
      const user = {
        fullName: 'fullName',
        singleKnownRole: Role.DizMember,
        isFromLocation: true,
      } as any as IRequestUser;
      await contractingService.handleContractSign(
        { ...proposal, isContractingComplete: true },
        true,
        user,
        proposalUrl,
      );
